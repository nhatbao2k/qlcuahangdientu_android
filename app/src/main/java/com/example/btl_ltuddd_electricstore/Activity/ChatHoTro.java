...
public class ChatHoTro extends AppCompatActivity {

    ImageView img_gui_anh, img_send;
    EditText edit_soan_tin;
    ListView lv;
    DatabaseReference mData;
    ChatHoTroAdapter chatHoTroAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        guiVanBan();
        guiAnh();
        chatHoTroAdapter = new ChatHoTroAdapter(ChatHoTro.this,chatArrayList, R.layout.dong_chat_ho_tro);
        lv.setAdapter(chatHoTroAdapter);
        HTChat();
        ...
        img_send.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(edit_soan_tin.getText().toString().trim().length() > 0){
                    ...
                    mData.child("HoTroChat").child(TenUser.ID).push().setValue(chat);
        ...
        if (requestCode == REQUEST_CHOSE_IMAGE && resultCode == RESULT_OK){
            try {
                ...
                // dua anh len firebase
                    ...
                    uploadTask.addOnSuccessListener(ChatHoTro.this, new OnSuccessListener<UploadTask.TaskSnapshot>() {
                        @Override
                        public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {// gui anh len firebase
                            ...
                            mData.child("HoTroChat").child(TenUser.ID).push().setValue(chat, new DatabaseReference.CompletionListener() {
                                @Override
    ...
    private void HTChat(){
        chatArrayList.clear();
        mData.child("HoTroChat").child(TenUser.ID).addChildEventListener(new ChildEventListener() {
            @Override
            public void onChildAdded(@NonNull DataSnapshot snapshot, @Nullable String previousChildName) {
                Chat chat = snapshot.getValue(Chat.class);
                chatArrayList.add(new Chat(chat.getId(), chat.getTen(), chat.getComment(), chat.getAnh()));
                chatHoTroAdapter.notifyDataSetChanged();
            }
